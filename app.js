/* globals jsPDF */

//~ document.getElementById("qrcode").innerHTML = "";
	 //~ var qrcode = new QRCode(document.getElementById("qrcode"), insta);
      //~ // Optional - set properties of the document
      //~ pdf.setProperties({
        //~ title   : flyer_title,
        //~ subject : footer_content,
        //~ author  : 'me',
        //~ creator : 'Flyer Builder & jsPDF'
      //~ });

var qrcodeI = new QRCode(document.getElementById("qrcode-insta"), {
	width : 100,
	height : 100
});
var qrcodeF = new QRCode(document.getElementById("qrcode-face"), {
	width : 100,
	height : 100
});
var qrcodeE = new QRCode(document.getElementById("qrcode-email"), {
	width : 100,
	height : 100
});
var qrcodeP = new QRCode(document.getElementById("qrcode-phone"), {
	width : 100,
	height : 100
});
var qrcodeW = new QRCode(document.getElementById("qrcode-web"), {
	width : 100,
	height : 100
});

function makeCode () {		
	var elText = document.getElementById("flyer-insta");
	if (!elText.value) { return;}
	qrcodeI.makeCode("https://www.instagram.com/"+elText.value);
	
	var elText = document.getElementById("flyer-face");
	if (!elText.value) { return;}
	qrcodeF.makeCode("https://www.facebook.com/"+elText.value);
	
	var elText = document.getElementById("flyer-email");
	if (!elText.value) { return;}
	qrcodeE.makeCode(elText.value);
	
	var elText = document.getElementById("flyer-phone");
	if (!elText.value) { return;}
	qrcodeP.makeCode(elText.value);
	
	var elText = document.getElementById("flyer-web");
	if (!elText.value) { return;}
	qrcodeW.makeCode(elText.value);
}

	  
$("#flyer-insta").on("blur", function () {makeCode();}).on("keydown", function (e) {makeCode();});
$("#flyer-face").on("blur", function () {makeCode();}).on("keydown", function (e) {makeCode();});
$("#flyer-email").on("blur", function () {makeCode();}).on("keydown", function (e) {makeCode();});
$("#flyer-phone").on("blur", function () {makeCode();}).on("keydown", function (e) {makeCode();});
$("#flyer-web").on("blur", function () {makeCode();}).on("keydown", function (e) {makeCode();});

	  
(function(API){
  API.textAlign = function(txt, options, x, y) {
    options = options ||{};
    // Use the options align property to specify desired text alignment
    // Param x will be ignored if desired text alignment is 'center'.
    // Usage of options can easily extend the function to apply different text
    // styles and sizes

    // Get current font size
    var fontSize = this.internal.getFontSize();

    // Get page width
    var pageWidth = this.internal.pageSize.width;

    // Get the actual text's width
    // You multiply the unit width of your string by your font size and divide
    // by the internal scale factor. The division is necessary
    // for the case where you use units other than 'pt' in the constructor
    // of jsPDF.

    var txtWidth = this.getStringUnitWidth(txt)*fontSize/this.internal.scaleFactor;

    if( options.align === 'center' ){

      // Calculate text's x coordinate
      x = ( pageWidth - txtWidth ) / 2;

    } else if( options.align === 'centerAtX' ){ // center on X value

      x = x - (txtWidth/2);

    } else if(options.align === 'right') {

      x = x - txtWidth;
    }

    // Draw text at x,y
    this.text(txt,x,y);
  };
  /*
    API.textWidth = function(txt) {
	    var fontSize = this.internal.getFontSize();
        return this.getStringUnitWidth(txt)*fontSize / this.internal.scaleFactor;
    };
*/

  API.getLineHeight = function(txt) {
    return this.internal.getLineHeight();
  };

})(jsPDF.API);



(function() {
  'use strict';

  /*
    Refs:
    http://mrrio.github.io/jsPDF/
    https://github.com/MrRio/jsPDF
    https://mrrio.github.io/jsPDF/doc/symbols/jsPDF.html
  */

  // some variables
  const agency_logo		 = {
      src: 'data:image/png;base64,',
      w  : 800,
      h  : 285
    },
    agency_name      = '5-min',
    agency_site_url  = 'RendezVousDans5Minutes.be - AfspraakIn5Minuten.be',
    footer_content   = agency_site_url,

    page_size        = 'a4',
    page_width       = 297, // mm
    page_margin      = 10,  // mm
    content_width    = page_width - (page_margin * 2), // available width for the content
    vspace = 10, // standard vertical space between elements

    // builder elements
    preview_container      = document.getElementById('pdf_preview'),
    update_preview_button  = document.getElementById('flyer_preview_btn'),
    download_button        = document.getElementById('flyer_download_btn'),
    //flyer_img_field        = document.getElementById('flyer-image'),

    // utility functions
    hex2rgb = function ( hex_string ) {
      if( /^#/.test(hex_string)) {
        hex_string = hex_string.substr(1);
      }
      if(hex_string.length === 3 ) {
        hex_string = hex_string.replace(/\w/, function(match) {
          return String(match) + String(match);
        });
      }
      return {
        red      : parseInt(hex_string.substr(0, 2), 16),
        green    : parseInt(hex_string.substr(2, 2), 16),
        blue     : parseInt(hex_string.substr(4, 2), 16)
      };
    },

    px2mm = function (pixel) {
      // px to inches
      var inches = pixel / 72;
      return inches * 25.4;
    },

    mm2px = function (millimeters) {
      // mm to inches
      var inches = millimeters / 25.4;
      return inches * 72;
    },

    // function to calculate and check img sizes
    imgSizes = function (img_w, img_h, img_mm_w) {
      /*
      img_w and img_h represent the original image size, in pixel
      img_mm_w is the desidered rendered image size, in millimeters
      */

      if( img_mm_w > content_width ) { // this should be never used...
        img_mm_w = content_width;
      }

      if(mm2px (img_mm_w) > img_w ) {
        throw 'The `img_mm_w` parameter is too big';
      }

      var img_mm_h = Math.round( (px2mm(img_h) * img_mm_w) / px2mm(img_w) );

      return {
        w            : img_mm_w,
        h            : img_mm_h,
        centered_x   : (page_width - img_mm_w) / 2
      };
    }
  ;

  // can_display_preview:
  // if true a preview of the PDF can be displayed in the iframe,
  // this value is set to false if the browser can't display the preview
  let can_display_preview = true;

  // preview can be displayed?
  if (navigator.msSaveBlob) { // older IE

    update_preview_button.prop('disabled', true);
    can_display_preview = false;

    preview_container.outerHTML =
      `<div class="no-blob">
        <div>
          The preview can't be displayed
        </div>
      </div>`;
  }

  try {

    // image reading
    // More info at https://developer.mozilla.org/en-US/docs/Web/API/FileReader/readAsDataURL
    // for more examples about file api
    // take a look at https://scotch.io/tutorials/use-the-html5-file-api-to-work-with-files-locally-in-the-browser

    let img_data = null;

    //pdf builder
    const createPDF = function (update_preview) {
      /*
      update_preview:
        ==> true: shows pdf online
        ==> false: downloads the pdf
      */
	makeCode();
      // variables
      let _y, _x, // drawing coord
        color_array, _string, lineHeight, y_correction,

        // form data
        flyer_title           = document.getElementById('flyer-title').value,
        flyer_title_size        = document.getElementById('flyer-title-size').value,
        flyer_title_color       = document.getElementById('flyer-title-color').value,
        flyer_description       = document.getElementById('flyer-description').value,

        // some colors:
        light_grey = 237,
        grey       = 128,
        black      = 0,
        white      = 255,

		
		

        pdf = new jsPDF('l', 'mm', page_size);

		
      _y = page_margin; // vertical starting point
	 
	  let QR_insta = document.querySelectorAll('#qrcode-insta img')[0].src;
	  let Vinsta =  document.getElementById('flyer-insta').value;
	  let QR_face = document.querySelectorAll('#qrcode-face img')[0].src;
	  let Vface =  document.getElementById('flyer-face').value;
	  let QR_email = document.querySelectorAll('#qrcode-email img')[0].src;
	  let Vemail =  document.getElementById('flyer-email').value;
	  let QR_phone = document.querySelectorAll('#qrcode-phone img')[0].src;
	  let Vphone =  document.getElementById('flyer-phone').value;
	  let QR_web = document.querySelectorAll('#qrcode-web img')[0].src;
	  let Vweb =  document.getElementById('flyer-web').value;
	
      // logo
      //var logo_sizes = imgSizes (agency_logo.w, agency_logo.h, 30);
      //pdf.addImage(agency_logo.src, 'PNG', logo_sizes.centered_x, _y, logo_sizes.w, logo_sizes.h);

      // fonts initializing
      pdf.setFont('helvetica');
      pdf.setFontType('bold');

      // main title
      color_array = hex2rgb(flyer_title_color);
      pdf.setTextColor(color_array.red, color_array.green, color_array.blue);

      pdf.setFontSize(flyer_title_size);

      lineHeight = px2mm(pdf.getLineHeight(flyer_title));

      //_y += (logo_sizes.h + vspace + lineHeight);
	  //_y += (vspace + lineHeight);
	  _y += 2*vspace

      pdf.textAlign(flyer_title, {align: 'center'}, 0, _y);

      _y += vspace;

    
      // description
      if(flyer_description) {
        pdf.setFontSize(12);
        pdf.setFont('helvetica');
        pdf.setFontType('italic');
        pdf.setTextColor(grey);

        var lineWidth = content_width - (page_margin * 2);
        _y += page_margin;

        var line_height = 10; // mm

        var description_lines = pdf.splitTextToSize(flyer_description, lineWidth);
        //pdf.text(page_margin, _y, description_lines); // doesn't allows to change line spacing

        for (var i = 0; i < description_lines.length; i++ ) {
          pdf.text(page_margin, _y, description_lines[i]);
          _y += line_height;
        }
		
		

      }

	  pdf.setFontSize(9);
      pdf.setFontType('normal');
      pdf.setTextColor(black);
	  
	  _y += 20;
	  
	  if(QR_insta.length != 0) {pdf.textAlign("Instagram", {align: 'centerAtX'}, 20+20, _y-3); pdf.addImage(QR_insta, 'png', 20, _y, 40, 40); pdf.textAlign(Vinsta, {align: 'centerAtX'}, 20+20, _y+50); }
	  if(QR_face.length != 0) { pdf.textAlign("Facebook", {align: 'centerAtX'}, 70+20, _y-3); pdf.addImage(QR_face, 'png', 70, _y, 40, 40);  pdf.textAlign(Vface, {align: 'centerAtX'}, 70+20, _y+50); }
	  if(QR_email.length != 0) { pdf.textAlign("Email", {align: 'centerAtX'}, 120+20, _y-3); pdf.addImage(QR_email, 'png', 120, _y, 40, 40);  pdf.textAlign(Vemail, {align: 'centerAtX'}, 120+20, _y+50); }
	  if(QR_phone.length != 0) { pdf.textAlign("Phone", {align: 'centerAtX'}, 170+20, _y-3); pdf.addImage(QR_phone, 'png', 170, _y, 40, 40);  pdf.textAlign(Vphone, {align: 'centerAtX'}, 170+20, _y+50); }
	  if(QR_web.length != 0) { pdf.textAlign("Web", {align: 'centerAtX'}, 220+20, _y-3); pdf.addImage(QR_web, 'png', 220, _y, 40, 40);  pdf.textAlign(Vweb, {align: 'centerAtX'}, 220+20, _y+50); }
	  
	  

      // footer
      _y = 200;
      
      pdf.textAlign(footer_content, {align: 'center'}, 0, _y);


      // output
      if(update_preview) {
        preview_container.setAttribute('src', pdf.output('datauristring') );
      } else {
        pdf.save('flyer ' + flyer_title + '.pdf');
      }

    }; // end createPDF

    // buttons
    update_preview_button.addEventListener('click', () => {
      createPDF(true);
    }, false);

    download_button.addEventListener('click', () => {
      createPDF(false);
    }, false);

  } catch (e) {
    console.log(e); // eslint-disable-line
  }

})();